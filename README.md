...
To validate and build a Markdown CV without creating a snapshot under `runs/`,
pass its path directly:

```bash
PYTHONPATH=src python3 -m cv_builder --input path/to/cv.md
```

This writes `cv.pdf` beside the source file. Override that location when needed:
...
Direct-file builds require YAML front matter in the source Markdown and use the
same validation, LaTeX template, Lua filters, Pandoc command, and XeLaTeX engine
...
This starts a server at `http://127.0.0.1:8007` (override with `--host` and
...
`runs/`, and rejects path traversal. On WSL, **Open in folder** converts the
selected directory to a Windows path and opens it with Windows Explorer when
available, falling back to the platform opener otherwise.
...
CV inputs, fallback metadata, cover letters, generated runs, PDFs, and DOCX files
are excluded from version control. Copy `example_input.md` to a local `input.md`
and keep personal or application-specific material outside the public repository.
