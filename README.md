...
Application-specific CVs generated by ChatGPT can instead be stored under
`applications/YYYY-MM-DD/company-role/cv.md`. A GitHub Actions workflow validates
and renders only the CVs changed by each push, then uploads their `cv.pdf` files
as a workflow artifact. See [Automated application CVs](docs/automated-applications.md)
for the file convention, artifact location, and matching local command.
...
The root `input.md`, fallback metadata, cover letters, and generated documents
remain excluded from version control for the private local workflow. Files placed
under `applications/` are intentionally versioned so GitHub Actions can build
them; review CV and job-description content before pushing, and never include
